...
#  Basis function labels (for correctly calculate integrals)
@dataclass
...
#  Prony fitting (for V_en and ERI calculation) & some basis coefficients TODO: check automatic basis in pyqed.
ETAS = np.array([
...
#  2D Overlap and Kinetic integrals (General)
...
#  TABULATION CACHE SYSTEM, used for integral calculation in the regime that fitting have large error
...
def _exact_ven_integrand(y, x, xA, yA, lxA, lyA, aA, xB, yB, lxB, lyB, aB, xN, yN, dz_sq):
    ...
    # TODO: or handle appropriately.
    if dist_sq < 1e-13:
        # If we are at the nucleus.
...
def _generate_exact_ven_table_entry(alphas, centers, labels, nuclei_tuples):
    ...
    global _VEN_SPLINES
    if _VEN_SPLINES: 
        return # Already initialized so skipp
...
#  Electron-Nuclear Attraction (V_en)
...
def _hermite_coefficients_general(l_a, l_b, alpha_a, alpha_b, A, B):
    # maybe TODO: could get a debug stuff
    pass
...
def eri_2d_cartesian_with_p(alphas, centers, labels, delta_z, dz_tol=None):
    ...
    if use_exact_bessel:
        ...
        for ki, kj, kk, kl in itertools.product(kinds, repeat=4):
            ...
            # Contraction TODO: consider more efficient way doing this? though won't involve much calculation in this branch in real runs
